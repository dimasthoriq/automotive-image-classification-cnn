...
input_folder = "C:\\Users\\Dimas Ahmad\\Downloads\\KP CNN\\spion"
output = "C:\\Users\\Dimas Ahmad\\Downloads\\KP CNN\\dataset spion"

splitfolders.ratio(input_folder, output=output, seed=24, ratio=(.7, .15, .15))
...
base_dir = "C:\\Users\\Dimas Ahmad\\Downloads\\KP CNN\\dataset spion"
train_dir = os.path.join(base_dir, 'train')
validation_dir = os.path.join(base_dir, 'val')
test_dir = os.path.join(base_dir, 'test')

train_0_dir = os.path.join(train_dir, 'GL, GA')
train_1_dir = os.path.join(train_dir, 'GX, XL7, SS')

validation_0_dir = os.path.join(validation_dir, 'GL, GA')
validation_1_dir = os.path.join(validation_dir, 'GX, XL7, SS')

test_0_dir = os.path.join(test_dir, 'GL, GA')
test_1_dir = os.path.join(test_dir, 'GX, XL7, SS')
...
print('total training GL/GA images:', len(os.listdir(train_0_dir)))
print('total training GX/XL7/SS images:', len(os.listdir(train_1_dir)))

print('total validation GL/GA images:', len(os.listdir(validation_0_dir)))
print('total validation GX/XL7/SS images:', len(os.listdir(validation_1_dir)))

print('total testing GL/GA images:', len(os.listdir(test_0_dir)))
print('total testing GX/XL7/SS images:', len(os.listdir(test_1_dir)))
...
os.chdir('C:\\Users\\Dimas Ahmad\\Downloads\\KP CNN\\dataset spion\\train\\GL, GA')
...
for current_dir, dirs, files in os.walk('.', topdown = True):
    ...
    for f in files:
        ...
        image_array = cv2.resize(image_array, (64, 64), interpolation=cv2.INTER_CUBIC)
...
os.chdir('C:\\Users\\Dimas Ahmad\\Downloads\\KP CNN\\dataset spion\\train\\GX, XL7, SS')
# Showing currently active directory
print('Currently active directory:')
print(os.getcwd())
print()
...
# ### Best number of neurons in output layer: 128 (Model 1)
# 
# RGB --> {32C3-P2} --> {64C3-P2} --> {128C3-P2} --> {256C3-P2} --> 128 --> 1
...
#Building model: RGB --> {32C3-P2} --> {64C3-P2} --> {128C3-P2} --> {256C3-P2} --> 128 --> 1
model_gen = []
for i in range(3):
  x = layers.Conv2D(32, 3, activation='relu')(img_input)
  x = layers.MaxPooling2D(2)(x)
...
# Our input feature map is 64x64x3: 64x64 for the image pixels, and 3 for the three color channels: R, G, and B
img_input = layers.Input(shape=(64, 64, 3))
...
mc = ModelCheckpoint('best_model_spion.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
...
history = model.fit(
      train_generator,
      steps_per_epoch=15,  # 263 images = batch_size * steps
      epochs=50,
...
acc = history.history['acc'][:22]
val_acc = history.history['val_acc'][:22]

# Retrieve a list of list results on training and validation data
# sets for each training epoch
loss = history.history['loss'][:22]
val_loss = history.history['val_loss'][:22]
...
glga_img_files = [os.path.join(train_0_dir, f) for f in train_0_fnames]
gxxl7ss_img_files = [os.path.join(train_1_dir, f) for f in train_1_fnames]
img_path = random.choice(glga_img_files + gxxl7ss_img_files)

img = load_img(img_path, target_size=(64, 64))  # this is a PIL image
x = img_to_array(img)  # Numpy array with shape (64, 64, 3)
x = x.reshape((1,) + x.shape)  # Numpy array with shape (1, 64, 64, 3)
...
saved_model = load_model('best_model_spion.h5')
